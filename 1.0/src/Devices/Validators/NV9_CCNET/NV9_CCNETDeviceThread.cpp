...
__fastcall TNV9_CCNETDeviceThread::TNV9_CCNETDeviceThread() : TDeviceThread(true)
{
...
__fastcall TNV9_CCNETDeviceThread::~TNV9_CCNETDeviceThread()
{
}

void __fastcall TNV9_CCNETDeviceThread::ProcessLoopCommand()
{
  ...
  try
  {
    ...
    if (DeviceState->StateCode != 0xff && DeviceState->StateCode != 0x00)
    {
      ...
      if (Log) Log->Write("NV9_CCNET Device has found.");
      ChangeDeviceState();
      Reset();
    }
    else
    {
        ...
        if (Log != NULL)
          if (Log) Log->Write("No NV9_CCNET Device Present...");
...
void TNV9_CCNETDeviceThread::ParseAnswer(int mode)
{
  try
  {
    ClearBuffer(data);
...
void TNV9_CCNETDeviceThread::calc_crc(BYTE byte)
{
   unsigned char i,c;
...
unsigned short TNV9_CCNETDeviceThread::Calc_CRC16NV9_CCNET(BYTE* DataBuf, unsigned short  BufLen)
{
   unsigned short i;
...
void TNV9_CCNETDeviceThread::SendPacket(BYTE command,int len_packet, BYTE* data)
{
  if (!Port->PortInit)
  {
  ...
  try
  {
    ...
    Calc_CRC16NV9_CCNET(Command,(unsigned short)(len_packet-2));
...
void TNV9_CCNETDeviceThread::GetStay()//POLL command
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::Reset()
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::EnableAll()
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::Disable()
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::CommandDisable()
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::Return(void)
{
  try
  {
    if (!Port->PortInit)
...
void TNV9_CCNETDeviceThread::Hold(void)
{
  try
  {
    if (!Port->PortInit)
...
int TNV9_CCNETDeviceThread::Stack()
{
  int stacked=0;
  try
  {
...
void TNV9_CCNETDeviceThread::SendACK()
{
...
void TNV9_CCNETDeviceThread::SendNAK()
{
  TSendType old = SendType;
  SendType = NotRecieveAnswer;
...
void TNV9_CCNETDeviceThread::PollingLoop()
{
    AnsiString mess;
    int count_inkass=0;
    int error=0;
...
std::string TNV9_CCNETDeviceThread::GetStateDescription()
{
  if (DeviceState)
  {
...
std::string TNV9_CCNETDeviceThread::GetMainFailureDescription(BYTE StatusCode)
{
    switch (StatusCode)
    {
...
std::string TNV9_CCNETDeviceThread::GetHardwareFailureDescription(BYTE StatusCode)
{
    switch (StatusCode)
    {
...
bool TNV9_CCNETDeviceThread::IsItYou()
{
    GetStay();
    if (DeviceState->StateCode == 0xFF)
        return false;
...
void TNV9_CCNETDeviceThread::ProcessOutCommand()
{
    try
    {

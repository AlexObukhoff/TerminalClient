...
__fastcall CPrim08TKThread::CPrim08TKThread() : TDeviceThread(true, false)
{
  PollingMode = false;
  BeginByte = 0x02;
  EndByte = 0x03;
...
__fastcall CPrim08TKThread::~CPrim08TKThread()
{
}

BYTE CPrim08TKThread::SendCommand(BYTE *Command, unsigned int CommandCount, BYTE *Answer, unsigned int& AnswerCount)
{
        int RepCount = 1;
        ...
        do
        {
                ...
                {
                  if (LoggingErrors)
                      Log->Write("Command sending tries over.");
                ...
                {//если ответ есть, смотрим его корректность и соответствие формату
                    ...
                    if (count > 1)
                        if (Command[5] != Answer[1])
                            {
                                Log->Write("Wrong answer check byte.");
                    ...
                    if (count > 1)
                    {
                        ...
                        {
                            Log->Write("Wrong answer checksum.");
...
int CPrim08TKThread::SendCommand(BYTE* Command, int CommandSize, BYTE*& Answer, int& AnswerSize, TSendType type, int mode)
{
    int result = -1;
    try
    {
...
bool CPrim08TKThread::CompareBCC(BYTE *Buffer, unsigned int Count)
{
  WORD BCC = 0;
  char CalcBCC[5];
  char AnswBCC[5];
...
bool CPrim08TKThread::NAKInAnswer(BYTE *Buffer)
{
    if (Buffer[0] == NAK)
            return true;
    else
...
bool CPrim08TKThread::ACKInAnswer(BYTE *Buffer)
{
    if (Buffer[0] == ACK)
            return true;
    else
...
bool CPrim08TKThread::CANInAnswer(BYTE *Buffer)
{
    if (Buffer[0] == CAN)
            return true;
    else
...
bool CPrim08TKThread::TruncateAnswer(BYTE *Buffer, DWORD &Count)
{
  unsigned int i;
  unsigned int size = 0;
  unsigned int pos = 0;
...
bool CPrim08TKThread::IsBufferEmpty(BYTE *Buffer, int size)
{
    int i;
    bool result = false;
...
int CPrim08TKThread::FindByteInBuffer(BYTE *Buffer, int BufferCount, BYTE Value, int Position)
{
  int result = -1;
  int i;
...
BYTE CPrim08TKThread::PrepareAnswer(BYTE *Buffer, unsigned int& Count)
{
...
BYTE CPrim08TKThread::CharToBYTE(BYTE ch)
{
  BYTE result = 0;
...
BYTE CPrim08TKThread::GetBYTE(BYTE *Buffer, int index)
{
  BYTE result = 0;
...
BYTE CPrim08TKThread::GetConstantStatusIndex()
{
  BYTE pos = 5;
  return pos;
}

BYTE CPrim08TKThread::GetCurrentStatusIndex()
{
  BYTE pos = 8;
  return pos;
}

BYTE CPrim08TKThread::GetResultCommandIndex()
{
  BYTE pos = 13;
  return pos;
}

BYTE CPrim08TKThread::GetPrinterStatusIndex()
{
  BYTE pos = 18;
  return pos;
}
...
WORD CPrim08TKThread::GetWORD(BYTE *Buffer, int index)
{
  WORD result = 0;
...
bool CPrim08TKThread::CheckStatus(BYTE *Buffer, unsigned int BufferCount)
{
  bool result = true;
  BYTE errorscount = 0;

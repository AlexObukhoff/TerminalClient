...
CStarTSP700::CStarTSP700(int ComPort,int BaudRate,TLogClass* _Log) : CPrinter(ComPort,_Log, "StarTSP700")
{
  DataLength = 1;
  /*delete Port;
  ...
  LoggingErrors = true;
  DeviceName = "StarTSP700";
}

CStarTSP700::~CStarTSP700()
{
}

void CStarTSP700::SendCommand()
{
...
void CStarTSP700::PrintString(AnsiString text)
{
  PrintLine(text);
  SendCommand();
}

void CStarTSP700::PrintCheck(TStringList* Text)
{
  char subtext[100];
...
void CStarTSP700::SendPacket(BYTE* command, int bytes_count,int datalen, BYTE* data, bool datafirst)
{
  if (!Port->PortInit)
    return;
  try
...
std::string CStarTSP700::GetStateDescription()
{
  return GetStatusDescription((BYTE)DeviceState->StateCode);
}

void CStarTSP700::SetCodePage()
{
   BYTE command[4];
   command[0] = 0x1B;
   command[1] = 0x1D;
...
void CStarTSP700::PrintLine(AnsiString text)
{
   BYTE command[2];
   command[0] = 0x0D;
   command[1] = 0x0A;
   ...
   SendType = NotRecieveAnswer;
   SendPacket(command, 2, datalen, (BYTE *)text.c_str(), true);
}

void CStarTSP700::PrintBigString(AnsiString text)
{
   ClearCommand();
   ClearAnswer();
   Command[0] = 0x1B;
...
void CStarTSP700::Feed(int count)
{
...
void CStarTSP700::ShriftOptionsEx(BYTE option1, BYTE option2)//true - заводской шрифт false - пользовательский
{
   BYTE command[4];
   command[0] = 0x1B;
   command[1] = 0x69;
...
std::string CStarTSP700::GetStatusDescription(BYTE StatusCode)
{
    std::string result = "";
 ...
 */
AnsiString CStarTSP700::EncodeBackslashes(AnsiString s)
{
        AnsiString out;
...
void CStarTSP700::PrintCheck(AnsiString text, std::string barcode)
{
  SetCodePage();
...
void CStarTSP700::PresenterAutoPush()
{
   BYTE command[4];
   command[0] = 0x1B;
   command[1] = 0x16;
...
void CStarTSP700::SetLeftMargin()
{
   BYTE command[3];
   command[0] = 0x1B;
   command[1] = 0x6C;
...
bool CStarTSP700::IsPrinterEnable()
{
  GetState();
  return PrinterEnable;
}

void CStarTSP700::PresenterTimer()
{
   BYTE command[4];
   command[0] = 0x1B;
   command[1] = 0x16;

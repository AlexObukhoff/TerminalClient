...
wp_k833::wp_k833(int ComPort, int BaudRate, TLogClass* _Log):CPrinter(0, _Log, "wp_k833")
{
    try
    {
        DeviceName = "wp_k833";
...
void wp_k833::Add(AnsiString Line)
{
        PrintData->Add(Line);
}
...
void wp_k833::Font(unsigned char f)
{
    if ((LoggingErrors)&&(Log))
...
void wp_k833::PrintCheck(AnsiString text, std::string barcode)
{
...
/*
AnsiString wp_k833::GetStateDescription(BYTE code)
{
    std::string result;
    switch(code)
    {
...
void wp_k833::GetState()
{
  BYTE cmdResult[32];
  memset(cmdResult, 0, 32);
  DWORD LastError, BytesCount;
  COMSTAT cs;
  std::string strStatus;

  PurgeComm(hCom,PURGE_TXCLEAR || PURGE_RXCLEAR || PURGE_TXABORT || PURGE_RXABORT);

  BYTE cmd1[2] = {0x1B, 0x76};
  WriteFile(hCom, cmd1, sizeof(cmd1), &dWritten, NULL);
  Sleep(1000);
  ClearCommError(hCom, &LastError, &cs);
  BytesCount = 0;

  memset(cmdResult, 0, 32);
  ReadFile(hCom, cmdResult, cs.cbInQue, &BytesCount, NULL);
  if(BytesCount > 0)
  {
   if((cmdResult[0] & 0x01) == 0)
   {
    strStatus = "Бумага скоро кончится";
    Log->Write(strStatus.c_str());
    DeviceState->StateCode = DSE_NEARENDPAPER;
    DeviceState->OutStateCode = DSE_NEARENDPAPER;
    DeviceState->StateDescription = strStatus;
   }

   if(/*cmdResult[0] & 0x04 ||*/ cmdResult[0] == 0x03)
   {
    strStatus = "Нет бумаги";
    Log->Write(strStatus.c_str());
  ...
  {
   strStatus = "Общая ошибка принтера";
   Log->Write(strStatus.c_str());
...
bool wp_k833::IsPrinterEnable()
{
  GetState();
  return PrinterEnable;
}

int wp_k833::Initialize()
{
    try
    {
      Init();

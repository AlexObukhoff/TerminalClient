...
TExpressPayDeviceClass::TExpressPayDeviceClass(int ComPort,TLogClass* _Log): CWatchDog(ComPort,_Log, "WatchdogExpressPay")
{
  delete Port;
...
TExpressPayDeviceClass::~TExpressPayDeviceClass()
{
}

void TExpressPayDeviceClass::Start()
{
  //включаем устройство, переход в режим ожидани€
  if (DeviceState->State != NotRun)
    return;
  DeviceState->State = Wait;
  DeviceThread = new TExpressPayDeviceThread();

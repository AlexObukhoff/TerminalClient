...
FocusScope {
	...
	onSelectTicket: {
		var m = ticketContainer.item.model
		...
		ticketContainer.item.model = m
		global.ticket = ticketContainer.item.model[aIndex].selected ? aTicket : null
	}
	...
	function onSearchDrawsFinished(aResult) {
		...
		if (Object.keys(aResult).length) {
			...
			ticketContainer.item.model = U.get6x36Tickets(aResult)
	...
	Component.onCompleted: {
		...
		ticketContainer.sourceComponent = ticketView
	}
}

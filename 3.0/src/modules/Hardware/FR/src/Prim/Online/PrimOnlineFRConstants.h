...
namespace CPrimOnlineFR
{
	...
	/// Билд прошивки, начиная с которой [полноценно] поддерживаются элементы ФФД 1.1 (теги 1226 и 1214).
	const double FFD11Build = 104.5;

	/// Билд прошивки, начиная с которой поддерживается установка признака способа расчёта через отдельный тег, а не в составе 1059.
	/// Точная версия, начиная с которой есть такая возможность - неизвестна. Поэтому используется последняя на данный момент версия ПО.
	const double PayOffSubjectMethodTypeBuild = 202.03;
	...
	inline double getActualFirmware(EFFD::Enum aFFD)
	{
		...
		if (aFFD == EFFD::F105) return 109.1;

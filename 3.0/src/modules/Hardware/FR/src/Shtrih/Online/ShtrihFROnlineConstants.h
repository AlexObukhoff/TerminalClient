	...
	namespace Errors
	{
		...
		public:
			Data()
			{
				...
				add('\xC9', "Перегрев печатающей головки", EType::Printer);
				add('\xCA', "Температура вне условий эксплуатации", EType::Printer);
				add('\xCB', "Неверный подытог чека", EType::FR);
				add('\xCE', "Лимит минимального свободного объема ОЗУ или ПЗУ на ККМ исчерпан", EType::FR);
				add('\xCF', "Неверная дата (Часы сброшены? Установите дату!)", EType::FR);
				add('\xD1', "Нет данных в буфере");
				add('\xD5', "Критическая ошибка при загрузке ERRxx", EType::FR);
				add('\xE0', "Ошибка связи с купюроприемником", EType::CashAcceptor);
				add('\xE1', "Купюроприемник занят", EType::CashAcceptor);
				add('\xE2', "Итог чека не соответствует итогу купюроприемника");
				add('\xE3', "Ошибка купюроприемника", EType::CashAcceptor);

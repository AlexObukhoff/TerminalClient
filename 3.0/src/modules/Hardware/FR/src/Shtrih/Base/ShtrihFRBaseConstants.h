...
namespace CShtrihFRBase
{
	...
	namespace Errors
	{
		...
		public:
			CData()
			{
				add('\x01', "Неисправен накопитель ФП 1, ФП 2 или часы",                 FRError::EType::FM);
				add('\x02', "Отсутствует ФП 1",                                          FRError::EType::FM);
				add('\x03', "Отсутствует ФП 2",                                          FRError::EType::FM);
				add('\x04', "Некорректные параметры в команде обращения к ФП",           FRError::EType::FM);
				add('\x05', "Нет запрошенных данных",                                    FRError::EType::FM);
				add('\x06', "ФП в режиме вывода данных",                                 FRError::EType::FM);
				add('\x07', "Некорректные параметры в команде для данной реализации ФП");
				add('\x08', "Команда не поддерживается в данной реализации ФП",          FRError::EType::FM);
				add('\x09', "Некорректная длина команды",                                FRError::EType::FM);
				add('\x0A', "Формат данных не BCD",                                      FRError::EType::FM);
				add('\x0B', "Неисправна ячейка памяти ФП при записи итога",              FRError::EType::FM);
				add('\x11', "Не введена лицензия",                                       FRError::EType::FM);
				add('\x12', "Заводской номер уже введен");
				add('\x13', "Текущая дата меньше даты последней записи в ФП",            FRError::EType::FM);
				add('\x14', "Область сменных итогов ФП переполнена");
				add('\x15', "Смена уже открыта",                                         FRError::EType::FM);
				add('\x16', "Смена не открыта",                                          FRError::EType::FM);
				add('\x17', "Номер первой смены больше номера последней смены");
				add('\x18', "Цата первой смены больше даты последней смены",             FRError::EType::FM);
				add('\x19', "Нет данных в ФП",                                           FRError::EType::FM);
				add('\x1A', "Область перерегистраций в ФП переполнена",                  FRError::EType::FM);
				add('\x1B', "Заводской номер не введен",                                 FRError::EType::FM);
				add('\x1C', "В заданном диапазоне есть поврежденная запись",             FRError::EType::FM);
				add('\x1D', "Повреждена последняя запись сменных итогов",                FRError::EType::FM);
				add('\x1E', "Область перерегистраций ФП переполнена");
				add('\x1F', "Отсутствует память регистров",                              FRError::EType::FM);
				add('\x20', "Переполнение денежного регистра при добавлении",            FRError::EType::FM);
				add('\x21', "Вычитаемая сумма больше содержимого денежного регистра");
				add('\x22', "Неверная дата",                                             FRError::EType::FM);
				add('\x23', "Нет записи активизации",                                    FRError::EType::FM);
				add('\x24', "Область активизаций переполнена",                           FRError::EType::FM);
				add('\x25', "Нет активизации с запрашиваемым номером",                   FRError::EType::FM);
				...
				add('\x57', "ЭКЛЗ: количество закрытых смен не совпадает с ФП",          FRError::EType::EKLZ);
				...
				add('\x64', "ФП отсутствует",                                            FRError::EType::FM);
				add('\x65', "Не хватает денег в секции");
				add('\x66', "Переполнение денег в секции");
				add('\x67', "Ошибка связи с ФП",                                         FRError::EType::FM);
				...
				add('\x70', "Переполнение ФП",                                           FRError::EType::FM);
				...
				add('\x78', "Замена ПО");
				add('\x79', "Замена ФП",                                                 FRError::EType::FM);
				...
				add('\x80', "Ошибка связи с ФП",                                         FRError::EType::FM);
				add('\x81', "Ошибка связи с ФП",                                         FRError::EType::FM);
				add('\x82', "Ошибка связи с ФП",                                         FRError::EType::FM);
				add('\x83', "Ошибка связи с ФП",                                         FRError::EType::FM);
				...
				add('\xA1', "ЭКЛЗ отсутствует",                                          FRError::EType::EKLZ);
				add('\xA2', "ЭКЛЗ: Некорректный формат или параметр команды");
				add('\xA3', "Некорректное состояние ЭКЛЗ");
				add('\xA4', "Авария ЭКЛЗ",                                               FRError::EType::EKLZ);
				add('\xA5', "Авария КС в составе ЭКЛЗ",                                  FRError::EType::EKLZ);
				add('\xA6', "Исчерпан временной ресурс ЭКЛЗ",                            FRError::EType::EKLZ);
				add('\xA7', "ЭКЛЗ переполнена",                                          FRError::EType::EKLZ);
				...
				add('\xB2', "ЭКЛЗ: Уже активизирована",                                  FRError::EType::EKLZ);
				...
				add('\xC4', "Несовпадение номеров смен",                                 FRError::EType::EKLZ);

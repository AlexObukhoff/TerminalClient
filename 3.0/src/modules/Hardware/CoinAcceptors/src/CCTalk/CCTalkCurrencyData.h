/* @file Данные валюты устройств на протоколе ccTalk. */
...
namespace CCCTalk
{
	struct SCurrencyData
	{
		...
		SCurrencyData(const char * aCountry, int aCode) : country(aCountry), code(aCode) {}
	};
	...
	private:
		void append(const QByteArray & aCountryCode, const char * aCountry, int aCode = Currency::NoCurrency) { mBuffer.insert(aCountryCode, SCurrencyData(aCountry, aCode)); }
	};

...
namespace CFR { namespace FiscalFields
{
	...
	#define ADD_FISCAL_FIELD(aField, aName, aType, ...) const int aName = [] () -> int { CFR::FiscalFields::CData::process(aField, CFR::FiscalFields::SData(CFR::FiscalFields::ETypes::aType, CFiscalSDK::aName, __VA_ARGS__)); return aField; } ();
	...
	// Налоги (на весь чек)
	ADD_FISCAL_FIELD(1102, TaxAmount02, VLN, QCoreApplication::translate("FiscalFields", "#tax_amount_02"), true);    // 1102 (Сумма НДС чека по ставке 18%).
	...
	ADD_FISCAL_FIELD(1106, TaxAmount06, VLN, QCoreApplication::translate("FiscalFields", "#tax_amount_06"), true);    // 1106 (Сумма НДС чека по расчетной ставке 18/118).

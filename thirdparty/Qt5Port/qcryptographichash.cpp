...
{
public:
    CCryptographicHash::Algorithm method;
    union {
...
/*!
  \class CCryptographicHash
  \inmodule QtCore

  \brief The CCryptographicHash class provides a way to generate cryptographic hashes.
  ...
  CCryptographicHash can be used to generate cryptographic hashes of binary or text data.

  Currently MD4, MD5, SHA-1, SHA-224, SHA-256, SHA-384, and SHA-512 are supported.
*/

/*!
  \enum CCryptographicHash::Algorithm
...
*/
CCryptographicHash::CCryptographicHash(Algorithm method)
    : d(new QCryptographicHash5Private)
{
    d->method = method;
    reset();
}
...
/*!
  ...
  \since 5.0
 */
bool CCryptographicHash::addData(QIODevice* device)
{
...
/*!
  ...
  \sa QByteArray::toHex()
*/
QByteArray CCryptographicHash::result() const
{
    if (!d->result.isEmpty())
        return d->result;

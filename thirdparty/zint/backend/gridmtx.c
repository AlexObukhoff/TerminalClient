...
void place_layer_id(char* grid, int size, int layers, int modules, int ecc_level)
{
	...
	int * layerid = intMalloc(layers + 1);
	int * id = intMalloc(modules * modules);
...
int grid_matrix(struct zint_symbol *symbol, uint8_t source[], int length)
{
	...
	free(utfdata); free(gbdata); free(grid);
	return 0;
}

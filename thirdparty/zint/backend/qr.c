...
void qr_binary(int datastream[], int version, int target_binlen, char mode[], int jisdata[], int length, int gs1, int est_binlen)
{
	...
	char * binary = charMalloc(est_binlen + 12);
	strcpy(binary, "");
...
void add_ecc(int fullstream[], int datastream[], int version, int data_cw, int blocks)
{
	...
	uint8_t * data_block = uint8Malloc(short_data_block_length + 2);
	uint8_t * ecc_block = uint8Malloc(ecc_block_length + 2);
	int * interleaved_data = intMalloc(data_cw + 2);
	int * interleaved_ecc = intMalloc(ecc_cw + 2);
...
int evaluate(uint8_t *grid, int size, int pattern)
{
	...
	char * local = charMalloc(size * size);
...
int apply_bitmask(uint8_t *grid, int size)
{
	...
	uint8_t * mask = uint8Malloc(size * size);
	uint8_t * eval = uint8Malloc(size * size);
...
int qr_code(struct zint_symbol *symbol, uint8_t source[], int length)
{
	...
	int * utfdata = intMalloc(length + 1);
	int * jisdata = intMalloc(length + 1);
	char * mode = charMalloc(length + 1);
	int * datastream;
	int * fullstream;
	uint8_t * grid;
	...
	switch(symbol->input_mode) {
		...
		default:
			...
			if(error_number != 0) { 
				free(utfdata); free(jisdata); free(mode);
				return error_number; 
			}
	...
	datastream = intMalloc(target_binlen + 1);
	fullstream = intMalloc(qr_total_codewords[version - 1] + 1);

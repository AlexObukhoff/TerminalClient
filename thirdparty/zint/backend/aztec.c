...
int aztec_text_process(uint8_t source[], const unsigned int src_len, char binary_string[], int gs1)
{
	...
	charmap = intMalloc(src_len * 2);
	typemap = intMalloc(src_len * 2);
...
int aztec(struct zint_symbol *symbol, uint8_t source[], int length)
{
    ...
    uint8_t * local_source = uint8Malloc(length + 1);
	...
	data_part = unsignedMalloc(data_blocks + 3);
	ecc_part = unsignedMalloc(ecc_blocks + 3);
